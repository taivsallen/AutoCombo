...
// 產生一份共同考卷，讓不同版本進行公平測試
const testSuite = Array.from({ length: NUM_TESTS }, () => generateRandomBoard());

console.log(`\n🚀 開始評測... 總共測試 ${NUM_TESTS} 個隨機盤面，目標 ${TARGET_COMBO} Combo\n`);

// 你可以在這裡替換你要測試的參數配置 (cfg)
const configTest = { maxSteps: 70, beamWidth: 30, maxNodes: 50000, stepPenalty: 50, potentialWeight: 100 };

// 執行評測
const report = runBenchmark("當前測試版本", beamSolve, testSuite, TARGET_COMBO, configTest);

console.log(`📊 [${report.versionName}] 表現：`);
console.table([report.metrics]);
console.log(`🏆 綜合評分: ${report.finalScore}\n`);